...
void solve(){
    // Observation [...]
    /* 
		Let this task be possible in x as minimum len;

		So, n*x * (m/(x+1)) + n*(m % (x+1)) should be more than or equal to k for this answer to be possible;

        So, just binary search for the solution
    */
 
    // Solution Here [...]
    int n, m, k; cin >> n >> m >> k;
    int low = 1, high = m, mid = (low + high)/2;
    while (high - low > 4 and high > low){
    	if (f(mid, n, m) >= k){
    ...
    for (int num = low; num <= high; num++){
    	if (f(num, n, m) >= k) { cout << num << endl; return;}
    }
