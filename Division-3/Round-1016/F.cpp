...
void solve(){
    ...
    int n, m; cin >> n >> m;
    vector<string> a(n);
    vector<vector<string>> neural(m, vector<string>(n));
    for (int i = 0; i < n; i++) cin >> a[i];
    for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) cin >> neural[i][j];

    vector<int> present(n); int minwrong = n;
	
	for (int i = 0; i < m; i++){
		int wrong = 0;
		for (int j = 0; j < n; j++) wrong += (neural[i][j] != a[j]), present[j] = max(present[j], (int) (neural[i][j] == a[j]));
		minwrong = min(minwrong, wrong);
	}

	if (accumulate(entire(present), 0) != n) cout << -1 << endl;
	else cout << n + 2*minwrong << endl;
}
