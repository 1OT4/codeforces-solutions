...
void solve(){
    ...
    */
    ...
    // Just get the number in base 4, then do these stuffs, and get it's position
 
    // Just get the binary representation of both a and b and use it to predict the 
    // ordered pairs used to form the number, and with this you get the number in base 4
    // convert the number in base 4 to decimal and you get the answer;
 
    // Solution Here [...]
    int n, q; cin >> n >> q;
    vector<pii> order{
        {0, 0}, {1, 1}, {1, 0}, {0, 1} // same order
    };
 
    map<pii, int> opp;
    opp[{0, 0}] = 0; opp[{1, 1}] = 1; opp[{1, 0}] = 2; opp[{0, 1}] = 3;
    ...
    for (int query = 0; query < q; query++){
        ...
        if (s == "->"){
            ...
            for (int i = 0; i < n; i++) bina.append(a % 2), a /= 2;
            for (int i = 0; i < n; i++) binb.append(b % 2), b /= 2;
            vector<int> d4;
            for (int i = 0; i < n; i++) d4.append(opp[{bina[i], binb[i]}]);
            int d = 0, p = 1;
            for (int i = 0; i < n; i++) d += p*d4[i], p *= 4;
            cout << d + 1 << endl;
 
        }else{
            int d; cin >> d; d--;
            vector<int> d4; int x = 0, y = 0;
            for (int i = 0; i < n; i++) d4.append(d % 4), d /= 4;
            for (int i = n-1; i > -1; i--){
                x += (1 << i)*order[d4[i]].ff; y += (1 << i)*order[d4[i]].ss;
            }
            cout << x + 1 << " " << y + 1 << endl;
        }
    }
