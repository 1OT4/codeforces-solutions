...
bool func(vector<int> &a, int mex, int k){
    // alright in this funciton we will check if the particular mex is possible or not
    // let's get this shit rolling
 
    int curlow = 0, segs = 0;
    vector<int> lesser(mex, -1);
    for (int i = 0; i <= a.size(); i++){
        if (curlow >= mex) segs++, curlow = 0;
        if (i < a.size()) {
            if (a[i] < mex){
                if (lesser[a[i]] != segs) curlow++, lesser[a[i]] = segs;
            } 
        }
    }
...
void solve(){
    ...
    while (high - low > 4){
        if (func(a, mid, k) == true) low = mid+1, mid = (low + high)/2;
        else high = mid, mid = (low + high)/2; 
    }
 
    for (int i = low; i <= high; i++){
        if (func(a, i, k) == false) { cout << i - 1 << endl; return; }
    }
    cout << high << endl;
}
