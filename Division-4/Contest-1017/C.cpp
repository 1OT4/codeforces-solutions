...
void solve(){
    ...
    for (int i = 0; i < n; i++) perm.append(grid[i][0]), done[grid[i][0]-1] = true;
    for (int j = 1; j < n; j++) perm.append(grid[n-1][j]), done[grid[n-1][j]-1] = true;

	int missing; for (int i = 0; i < 2*n; i++) if (!done[i]) {missing = i + 1; break;}
	cout << missing << " ";
