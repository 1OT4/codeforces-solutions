...
void divisions(vector<vector<int>> &pos, vector<pair<int, int>> &factors, int l, int r, int idx, int num, vector<pair<int, int>> &final){
    int n = num;
    if (idx >= factors.size()){
        auto index = lower_bound(entire(pos[n]), l);
        if (index != pos[n].end() and *index <= r) final.push_back({*index, num});
        return;
    }
    for (int i = 0; i <= factors[idx].ss; i++) divisions(pos, factors, l, r, idx+1, n, final), n *= factors[idx].ff;
}
...
void solve(){
    ...
    // Solution Here [...]
    int n, q; cin >> n >> q;
    vector<int> a(n); int maxa = 0;
    for (int i = 0; i < n; i++) cin >> a[i], maxa = max(maxa, a[i]);

    for (int i = 0; i < n; i++) pos[a[i]].append(i);


    for (int query = 0; query < q; query++){
    	int k, l, r; cin >> k >> l >> r; l--; r--;
        if (k == 1) { cout << r - l + 1 << endl; continue; }
    	vector<pair<int, int>> factors = primeFactorize(k);
        vector<pair<int, int>> final;
        divisions(pos, factors, l, r, 0, 1, final); sort(entire(final));
        int ans = 0, lastidx = l;
        for (auto it = final.begin(); it != final.end(); it++){
            // cout << " (" << it->first << " " << it->second << "), ";
            ans += k * ((it->first) - lastidx);
            while (k % (it -> ss) == 0) k /= (it -> ss);
            // dbg(ans, k, lastidx);
            lastidx = (it -> first);
        } ans += k * (r - lastidx + 1); //cout << endl; 
        cout << ans << endl;
    }
