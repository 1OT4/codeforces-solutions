...
void solve(){
    ...
    for (int i = 1; i < a.size(); i++){
    	if (a[i] == a[i-1]) curlen++;
    	else seglens.append(curlen*2), curlen = 1;
    } seglens.append(curlen*2);

    vector<int> seglen2; curlen = 1;
    for (int i = 1; i < b.size(); i++){
    	if (b[i] == b[i-1]) curlen++;
    	else seglen2.append(curlen), curlen = 1;
    } seglen2.append(curlen);

    // for (int i = 0; i < seglens.size(); i++) cout << seglens[i] << " "; cout << endl;
    // for (int i = 0; i < seglen2.size(); i++) cout << seglen2[i] << " "; cout << endl;

    if (seglens.size() != seglen2.size()) cout << "NO" << endl;
    else{
    	for (int i = 0; i < seglens.size(); i++){
    		if (seglens[i] < seglen2[i] or seglens[i]/2 > seglen2[i]) {cout << "NO" << endl; return;}
    	} cout << "YES" << endl;
    }
}
