...
void solve(){
    // Observation [...]
    /* 
		At every point of time we'll keep track of the sum of all the elements of the array;
     */
 
    // Solution Here [...]
    int q; cin >> q;
    int rizz = 0, len = 0, sum = 0, sign = +1;
    deque<int> a;
    while (q--){
    	int s; cin >> s;
    	if (s == 1){
    		if (len == 0) continue;
    		if (sign == +1) { rizz = rizz - a.back()*len + sum; a.push_front(a.back()); a.pop_back(); }
    		else { rizz = rizz - a.front()*len + sum; a.push_back(a.front()); a.pop_front(); }
    	}else if (s == 2){
    		if (len == 0) continue;
    		rizz = -(rizz - sum*(len + 1)); sign *= -1;
    	}else{
    		int k; cin >> k;
    		rizz += k*(len+1); len++; sum += k; 
    		if (sign == +1) a.push_back(k);
    		else a.push_front(k);
    	}
    	cout << rizz << endl;
    }
